...
description = "Biber 3+4axis safe rapids (LinuxCNC)";
vendor = "Machine Magic Berlin";
vendorUrl = "http://www.linuxcnc.org";
legal = "CC-BY-SA 4.0";
certificationLevel = 2;
minimumRevision = 24000;

longDescription = "Custom post for Biber VMC 3+4axis.";
...
minimumChordLength = spatial(0.15, MM);
...
properties = {
  useCustomTCL: true, //custom tool change locatino
...
propertyDefinitions = {
  useCustomTCL: {title:"Custom Tool Change Location", description: "Custom Tool Change Location", group: 0, type:"boolean"},
...
function onOpen() {
  ...
  if (true) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:false, axis:[-1, 0, 0], range:[-90, 90], preference:1});
    var bAxis = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[-90, 90], preference:1});
...
function setWorkPlane(abc) {
  ...
  writeComment("currentWorkPlaneABC " + currentWorkPlaneABC);

 // if (abcFormat.format(abc.x) == 90) {
 //   writeComment("A 90, Horizontal op from Front Side ");
 // }
 //
 // if (abcFormat.format(abc.x) == -90) {
 //   writeComment("A -90, Horizontal op from Back Side ");
 // }
 //
 // if (abcFormat.format(abc.y) == 90) {
 //   writeComment("B 90, Horizontal op from Right Side ");
 // }
 //
 // if (abcFormat.format(abc.y) == -90) {
 //   writeComment("B -90, Horizontal op from Left Side ");
 // }

 if ((abcFormat.format(abc.x) == 0) && (abcFormat.format(abc.y) == -90)) {
   writeComment("Set G19, Workplane YZ, approach from Left Side");
   writeBlock(gPlaneModal.format(19));
   writeBlock(mFormat.format(299)); //lower horizontal aggregate
 }

 if ((abcFormat.format(abc.x) == 0) && (abcFormat.format(abc.y) == 90)) {
   writeComment("Set G19, Workplane YZ, approach from Right Side");
   writeBlock(gPlaneModal.format(19));
   writeBlock(mFormat.format(299));
 }

 if ((abcFormat.format(abc.x) == 90) && (abcFormat.format(abc.y) == 0)) {
   writeComment("Set G18, Workplane YZ, approach from Front Side");
   writeBlock(gPlaneModal.format(18));
   writeBlock(mFormat.format(299));
 }

if ((abcFormat.format(abc.x) == -90) && (abcFormat.format(abc.y) == 0)) {
    writeComment("Set G18, Workplane YZ, approach from Back Side");
    writeBlock(gPlaneModal.format(18));
    writeBlock(mFormat.format(299));
  }

if ((abcFormat.format(abc.x) == 0) && (abcFormat.format(abc.y) == 0)) {
  writeComment("Set G17, Workplane XY, approach from Top, retreat hor aggregate");
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    if (insertToolCall && properties.useCustomTCL) {   // fly to custom tool change location
        writeComment("useCustomTCL start");
        writeBlock(gFormat.format(53),gFormat.format(0), zOutput.format(0));
  	    writeBlock(gFormat.format(53),gFormat.format(0), xOutput.format(500), yOutput.format(300));
        writeComment("useCustomTCL end");
      }

    // retract to safe plane
    writeRetract(Z);
    writeComment("writeRetract Z");
  ...
  if (insertToolCall) {
    ...
    if (tool.number > 99999) {
      warning(localize("Tool number exceeds maximum value of 99999"));
    }
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    gMotionModal.reset();
    writeBlock(gPlaneModal.format(17)); //SHU: why do we need this here?
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      //SHU: move X and Y separately to prevent dogleg crashing the horizontal aggregate
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
...
function onCycle() {
  ...
  if ((abcFormat.format(abc.x) == 0) && (abcFormat.format(abc.y) == -90)) {
    writeComment("onCycle set G19, Workplane YZ, approach from Left Side");
    writeBlock(gPlaneModal.format(19));
  }

  if ((abcFormat.format(abc.x) == 0) && (abcFormat.format(abc.y) == 90)) {
    writeComment("onCycle set G19, Workplane YZ, approach from Right Side");
    writeBlock(gPlaneModal.format(19));
  }

  if ((abcFormat.format(abc.x) == 90) && (abcFormat.format(abc.y) == 0)) {
    writeComment("onCycle set G18, Workplane YZ, approach from Front Side");
    writeBlock(gPlaneModal.format(18));
  }

 if ((abcFormat.format(abc.x) == -90) && (abcFormat.format(abc.y) == 0)) {
     writeComment("onCycle set G18, Workplane YZ, approach from Back Side");
     writeBlock(gPlaneModal.format(18));
   }
...
function onCyclePoint(x, y, z) {
  ...
  */
  ...
  if (isFirstCyclePoint()) {
    ...
    // case "drilling": // disabled to force G1 output as LinuxCNC does not support G81 in positive direction, for ops like backboring or approach from X- in G19.
    //   writeBlock(
    ...
    // case "deep-drilling": // disabled to force G1 output
    //   if (P > 0) {
    //     expandCyclePoint(x, y, z);
    //   } else {
    //     writeBlock(
